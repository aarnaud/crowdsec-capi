...
| `internal/auth` | HS256 JWT (sign/verify), bcrypt password, chi Bearer middleware |
| `internal/api/router.go` | Assembles all routes; admin auth middleware (Basic Auth or Bearer API key) |
...
### Admin auth
Two accepted methods (checked in order):
1. `Authorization: Bearer <cfg.Admin.APIKey>` (if api_key is set)
2. HTTP Basic Auth with `cfg.Admin.Username` / `cfg.Admin.Password`

Password auto-generated at startup if empty (logged at WARN level).
